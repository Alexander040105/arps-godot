...
Note: for macos, you will have to build the binary as a `.dylib` in a `arps.framework` folder. The framework folder should also have a `Resources` folder with a file called `Info.plist`. Without this file, signing will fail.
...
```sh
bin/arps.macos.template_debug.framework/arps.macos.template_release
bin/arps.ios.template_debug.framework/arps.ios.template_release.arm64.dylib
